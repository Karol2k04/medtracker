...
class DoctorNoteTests(APITestCase):
    def setUp(self):
        ...
        self.medication2 = Medication.objects.create(
            name="Another Med", dosage_mg=50, prescribed_per_day=1
        )
    ...
    def test_create_note_success(self):
        ...
        self.assertEqual(
            response.data["text"], "Patient shows good response to medication"
        )
    ...
    def test_create_note_invalid_medication(self):
        ...
        url = reverse("doctornote-list")
        data = {"medication": 99999, "text": "Some note text"}
        response = self.client.post(url, data, format="json")
    ...
    def test_list_notes(self):
        ...
        # Create some notes
        DoctorNote.objects.create(medication=self.medication, text="First note")
        DoctorNote.objects.create(medication=self.medication2, text="Second note")
    ...
    def test_retrieve_note(self):
        ...
        note = DoctorNote.objects.create(
            medication=self.medication, text="Detailed medical observation"
        )
    ...
    def test_delete_note(self):
        ...
        note = DoctorNote.objects.create(
            medication=self.medication, text="Note to be deleted"
        )
    ...
    def test_update_note_not_allowed_put(self):
        ...
        note = DoctorNote.objects.create(
            medication=self.medication, text="Original text"
        )

        url = reverse("doctornote-detail", kwargs={"pk": note.id})
        data = {"medication": self.medication.id, "text": "Updated text"}
    ...
    def test_notes_for_specific_medication(self):
        ...
        _note1 = DoctorNote.objects.create(
            medication=self.medication, text="Note for medication 1"
        )
        _note2 = DoctorNote.objects.create(
            medication=self.medication, text="Another note for medication 1"
        )
        _note3 = DoctorNote.objects.create(
            medication=self.medication2, text="Note for medication 2"
        )
        ...
        med2_notes = [
            n for n in response.data if n["medication"] == self.medication2.id
        ]
    ...
    def test_created_at_is_auto_generated(self):
        ...
        note = DoctorNote.objects.create(medication=self.medication, text="Test note")
