...
class MedicationModelTests(TestCase):
    ...
    def test_adherence_rate_with_logs(self):
        ...
        DoseLog.objects.create(medication=med, taken_at=timezone.now(), was_taken=True)
    ...
    def test_expected_doses(self):
        med = Medication.objects.create(
            name="Test", dosage_mg=100, prescribed_per_day=2
        )
    ...
    def test_adherence_rate_over_period(self):
        med = Medication.objects.create(
            name="Test", dosage_mg=100, prescribed_per_day=1
        )
